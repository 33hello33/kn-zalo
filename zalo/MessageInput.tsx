      ...
      try {
        ...
        if (activeAccountId) {
          ...
          if (dbRes?.success && dbRes?.contact) {
            ...
            const nameToUse = c.alias || c.display_name || c.displayName || c.zalo_name || c.phone || phone;
            setContactCardSuggestion({
              userId: c.contact_id || c.userId || '',
        ...
        if (foundUser?.userId || foundUser?.uid || foundUser?.id) {
          ...
          // Try getUserInfo for detailed info (avatar, name, alias)
          let displayName = foundUser.alias || foundUser.displayName || foundUser.name || foundUser.zaloName || uid;
          ...
          try {
            ...
            if (infoRes?.response) {
              ...
              displayName = info.alias || info.displayName || info.name || displayName;
              avatarUrl = info.avatar || info.avatarUrl || info.avatar_url || avatarUrl;
            }
          } catch { /* best-effort */ }

          // Check if local store has alias for this user
          if (activeAccountId) {
            try {
              const localContacts = useChatStore.getState().contacts[activeAccountId] || [];
              const localC = localContacts.find((item: any) => (item.contact_id || item.userId || item.id) === uid);
              if (localC?.alias) {
                displayName = localC.alias;
              }
            } catch {}
          }
